...
| `Compiler/Architecture/JoomlaFour/Module/Helper::get()` | The database-aware helper renders its trait and interface with the shared indentation utility. | `Indent::_()` is called without importing the utility, so PHP resolves a nonexistent class in the module namespace. |
...
| `Componentbuilder/Extrusion/Helper/Builder::addFields()` | The first configured list field remains marked for list, search, and filter behavior. | `array_search()` result `0` is treated as false, so the first list field receives zeroed flags. |
...
| `Componentbuilder/Power/Parser::getClassProperties()` | Conventional `protected static ?string $name` ordering retains the property name, visibility, type, and static modifier. | The expression recognizes `static` only before visibility, so the standard modifier order is dropped. |
...
| `Componentbuilder/Abstraction/Console/Package/Get::resolveRepoByGuid()` | An unresolved repository GUID warns through the lazily resolved message bus and falls back to the default repository. | The nullable `$message` property is dereferenced before initialization, causing an error instead of a warning and fallback. |
