 ...
 * @since  1.6
 */
class Component_custom_admin_viewsModel extends AdminModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_componentbuilder/assets/css/component_custom_admin_views.css'
 	];
	...
	protected array $scripts = [
		...
		'media/com_componentbuilder/js/component_custom_admin_views.js'
 	];
	 ...
	 * @throws  \Exception
	 */
	public function getTable($type = 'component_custom_admin_views', $prefix = 'Administrator', $config = [])
	{
	...
	public function getVDM(): string
	{
		if (!isset($this->vastDevMod))
		{
			...
			if (($vdm = SessionHelper::get('component_custom_admin_views__' . $id)) !== null)
			{
				$this->vastDevMod = $vdm;
			}
			else
			{
				...
				SessionHelper::set($this->vastDevMod, 'component_custom_admin_views__' . $id);
				SessionHelper::set('component_custom_admin_views__' . $id, $this->vastDevMod);
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('component_custom_admin_views.delete', 'com_componentbuilder.component_custom_admin_views.' . (int) $record->id);
	}
