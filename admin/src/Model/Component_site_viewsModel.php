 ...
 * @since  1.6
 */
class Component_site_viewsModel extends AdminModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_componentbuilder/assets/css/component_site_views.css'
 	];
	...
	protected array $scripts = [
		...
		'media/com_componentbuilder/js/component_site_views.js'
 	];
	 ...
	 * @throws  \Exception
	 */
	public function getTable($type = 'component_site_views', $prefix = 'Administrator', $config = [])
	{
	...
	public function getVDM(): string
	{
		if (!isset($this->vastDevMod))
		{
			...
			if (($vdm = SessionHelper::get('component_site_views__' . $id)) !== null)
			{
				$this->vastDevMod = $vdm;
			}
			else
			{
				...
				SessionHelper::set($this->vastDevMod, 'component_site_views__' . $id);
				SessionHelper::set('component_site_views__' . $id, $this->vastDevMod);
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('component_site_views.delete', 'com_componentbuilder.component_site_views.' . (int) $record->id);
	}
