...
/**
 * Reads one layout source file into the View registry.
 *
 * JCB's layout table holds the same file twice over: php_view carries the PHP
 * above the closing tag, layout carries the HTML after it, and add_php_view says
 * whether the PHP part exists at all. This reader is therefore a thin shell
 * around the split, whose only decisions are what the layout is called and what
 * is worth recording when a file yields nothing.
 *
 ...
 * @since 6.1.6
 */
final class Layout implements ReaderInterface
{
	/**
	 * The View Registry.
	 *
	...
	/**
	 * Read one layout source file into the View registry.
	 *
	 * @param   string       $path  Absolute path to the layout file.
	 * @param   string|null  $name  The layout name, derived from the file name when null.
	 *
	...
	public function read(string $path, ?string $name = null): bool
	{
		...
		$this->view->set($base . '.layout', $parts['html']);
		...
		$this->report->set($base . '.layout', strlen($parts['html']));
