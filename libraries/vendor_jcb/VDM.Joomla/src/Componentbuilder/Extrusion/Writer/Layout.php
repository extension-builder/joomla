...
/**
 * Writes the JCB layout definitions the view reader split apart.
 *
 * A source layout file is two artifacts in one: the PHP above the final closing
 * tag and the markup after it. Both are stored raw, because the layout table
 * declares base64 storage on each column and the Data pipeline applies it.
 *
 * @since 6.1.6
 */
final class Layout extends Writer
{
	/**
	 * The View Registry.
	 *
	...
	/**
	 * Write every layout the reader classified.
	 *
	...
	public function write(): int
	{
		...
		foreach ((array) $entries as $key => $entry)
		{
			...
			$definition->guid = $this->guid->derive([$this->option(), 'layout', $name]);
			$definition->name = $name;
			$definition->description = $name . ' (extruded)';
			$definition->php_view = (string) ($entry['php_view'] ?? '');
			$definition->layout = (string) ($entry['layout'] ?? '');
		...
		$this->report->set('counts.layout', $written);
