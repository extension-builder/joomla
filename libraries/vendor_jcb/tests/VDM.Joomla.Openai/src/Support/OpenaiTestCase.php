...
namespace VDM\Joomla\Openai\Tests\Support;
...
/**
 * Shared deterministic composition for OpenAI endpoint tests.
 *
 * @since  6.1.6
 */
#[UsesClass(Api::class)]
...
abstract class OpenaiTestCase extends TestCase
{
	/**
	 ...
	 *
	 * @return  array{0: Api, 1: RecordingHttpTransport}
	 * @since   6.1.6
	 */
	protected function createEndpoint(string $class, array $responses = [[200, '{}']]): array
	{
		...
		{
			$queued[] = RecordingHttpTransport::cmsResponse($status, $body);
		}

		$http = new Http('test-token', 'test-organization');
		$transport = (new RecordingHttpTransport(...$queued))->attachTo($http);
		$endpoint = new $class(
			$http,
			new Uri('https://openai.example.test', 'v-test'),
			new Response()
		);
	...
	/**
	 * Assert the common OpenAI request envelope.
	 *
	 * @param   RecordingHttpTransport  $transport  The recording boundary.
	 * @param   string                  $method     Expected HTTP method.
	 * @param   string                  $path       Expected API path.
	 * @param   int                     $index      Request index.
	 *
	 * @return  void
	 * @since   6.1.6
	 */
	protected function assertOpenaiRequest(
	...
	): void
	{
		...
		$this->assertSame('https://openai.example.test/v-test' . $path, $request['uri']);
		$this->assertSame('Bearer test-token', $request['headers']['Authorization'] ?? null);
		$this->assertSame('test-organization', $request['headers']['OpenAI-Organization'] ?? null);
		$this->assertSame('JoomlaOpenai/3.0', $request['userAgent']);
	}
}
